import { test, expect } from '@playwright/test';

const API_BASE = 'http://localhost:8080';

test('clicking a unit shows its detail page', async ({ page }) => {
  await page.goto('/factions/NEC');
  await expect(page.locator('.datasheet-list')).toBeVisible();

  const firstUnit = page.locator('.datasheet-item').first();
  const unitName = await firstUnit.textContent();
  await firstUnit.getByRole('link').click();

  await expect(page).toHaveURL(/\/datasheets\/\d{9}$/);
  await expect(page.locator('.unit-name')).toHaveText(unitName!);
});

test('unit detail shows stats, weapons, and costs', async ({ page }) => {
  await page.goto('/factions/NEC');
  await expect(page.locator('.datasheet-list')).toBeVisible();

  await page.locator('.datasheet-item').first().getByRole('link').click();
  await expect(page.locator('.unit-name')).toBeVisible();

  await expect(page.locator('.stats-table')).toBeVisible();
  const statsRows = page.locator('.stats-row');
  expect(await statsRows.count()).toBeGreaterThan(0);
});

test('unit detail stats match API response', async ({ page, request }) => {
  await page.goto('/factions/NEC');
  await expect(page.locator('.datasheet-list')).toBeVisible();

  await page.locator('.datasheet-item').first().getByRole('link').click();
  await expect(page.locator('.unit-name')).toBeVisible();

  const url = page.url();
  const datasheetId = url.split('/datasheets/')[1];

  const apiResponse = await request.get(`${API_BASE}/api/datasheets/${datasheetId}`);
  expect(apiResponse.ok()).toBeTruthy();
  const detail = await apiResponse.json();

  await expect(page.locator('.unit-name')).toHaveText(detail.datasheet.name);

  if (detail.profiles.length > 0) {
    const firstRow = page.locator('.stats-row').first();
    await expect(firstRow.locator('.stat-m')).toHaveText(detail.profiles[0].movement);
    await expect(firstRow.locator('.stat-t')).toHaveText(detail.profiles[0].toughness);
    await expect(firstRow.locator('.stat-sv')).toHaveText(detail.profiles[0].save);
    await expect(firstRow.locator('.stat-w')).toHaveText(String(detail.profiles[0].wounds));
    await expect(firstRow.locator('.stat-ld')).toHaveText(detail.profiles[0].leadership);
    await expect(firstRow.locator('.stat-oc')).toHaveText(String(detail.profiles[0].objectiveControl));
  }

  if (detail.costs.length > 0) {
    await expect(page.locator('.costs-table')).toBeVisible();
    const firstCostRow = page.locator('.cost-row').first();
    await expect(firstCostRow.locator('.cost-value')).toHaveText(String(detail.costs[0].cost));
  }
});

test('datasheet detail shows abilities section', async ({ page, request }) => {
  await page.goto('/factions/NEC');
  await expect(page.locator('.datasheet-list')).toBeVisible();

  await page.locator('.datasheet-item').first().getByRole('link').click();
  await expect(page.locator('.unit-name')).toBeVisible();

  const url = page.url();
  const datasheetId = url.split('/datasheets/')[1];

  const apiResponse = await request.get(`${API_BASE}/api/datasheets/${datasheetId}`);
  const detail = await apiResponse.json();

  const abilitiesWithNames = detail.abilities.filter((a: { name: string | null }) => a.name);
  if (abilitiesWithNames.length > 0) {
    await expect(page.locator('.abilities-list')).toBeVisible();
    const items = page.locator('.ability-item');
    await expect(items).toHaveCount(abilitiesWithNames.length);
  }
});

test('datasheet detail shows unit-specific stratagems', async ({ page, request }) => {
  await page.goto('/factions/NEC');
  await expect(page.locator('.datasheet-list')).toBeVisible();

  await page.locator('.datasheet-item').first().getByRole('link').click();
  await expect(page.locator('.unit-name')).toBeVisible();

  const url = page.url();
  const datasheetId = url.split('/datasheets/')[1];

  const apiResponse = await request.get(`${API_BASE}/api/datasheets/${datasheetId}`);
  const detail = await apiResponse.json();

  if (detail.stratagems.length > 0) {
    await expect(page.locator('.unit-stratagems-list')).toBeVisible();
    const items = page.locator('.unit-stratagem-item');
    await expect(items).toHaveCount(detail.stratagems.length);
  }
});

test('weapons table includes abilities column', async ({ page }) => {
  await page.goto('/factions/NEC');
  await expect(page.locator('.datasheet-list')).toBeVisible();

  await page.locator('.datasheet-item').first().getByRole('link').click();
  await expect(page.locator('.unit-name')).toBeVisible();

  const weaponsTable = page.locator('.weapons-table');
  if (await weaponsTable.isVisible()) {
    const headers = weaponsTable.locator('thead th');
    await expect(headers.last()).toHaveText('Abilities');

    const abilitiesCells = page.locator('.weapon-abilities');
    expect(await abilitiesCells.count()).toBeGreaterThan(0);
  }
});

test('datasheet detail shows wargear options', async ({ page, request }) => {
  // Navigate to a unit known to have wargear options (Overlord)
  await page.goto('/datasheets/000000523');
  await expect(page.locator('.unit-name')).toBeVisible();

  const apiResponse = await request.get(`${API_BASE}/api/datasheets/000000523`);
  const detail = await apiResponse.json();

  if (detail.options.length > 0) {
    await expect(page.locator('.wargear-options-list')).toBeVisible();
    const items = page.locator('.wargear-option-item');
    await expect(items).toHaveCount(detail.options.length);
  }
});
