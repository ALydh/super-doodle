...
test('detachment dropdown matches API data', async ({ page, request }) => {
  ...
  await expect(page.locator('.detachment-select')).toBeVisible();

  const apiRes = await request.get(`${API_BASE}/api/factions/${FACTION_ID}/detachments`);
  expect(apiRes.ok()).toBeTruthy();
  const detachments = await apiRes.json();

  const options = page.locator('.detachment-select').locator('option');
  await expect(options).toHaveCount(detachments.length);

  for (const det of detachments) {
    await expect(page.locator('.detachment-select').locator(`option[value="${det.detachmentId}"]`)).toBeAttached();
  }
});
...
test('available units match faction datasheets', async ({ page, request }) => {
  ...
  const pickerItems = page.locator('.unit-picker-item');
...
test('detachment abilities appear when selecting a detachment', async ({ page, request }) => {
  ...
  await page.locator('.detachment-select').selectOption(DETACHMENT_ID);

  await expect(page.locator('.detachment-abilities-section')).toBeVisible();
  const toggle = page.locator('.detachment-abilities-toggle');
  await expect(toggle).toBeVisible();

  await toggle.click();
  await expect(page.locator('.detachment-abilities-list')).toBeVisible();

  const apiRes = await request.get(`${API_BASE}/api/detachments/${DETACHMENT_ID}/abilities`);
  expect(apiRes.ok()).toBeTruthy();
  const abilities = await apiRes.json();

  const items = page.locator('.detachment-ability-item');
...
test('detachment stratagems appear filtered by selected detachment', async ({ page, request }) => {
  ...
  await expect(page.locator('.detachment-stratagems-section')).toBeVisible();
  const toggle = page.locator('.detachment-stratagems-toggle');
  await expect(toggle).toBeVisible();

  await toggle.click();
  await expect(page.locator('.detachment-stratagems-list')).toBeVisible();
  ...
  const items = page.locator('.detachment-stratagem-item');
...
test('wargear options appear for units that have them', async ({ page, request }) => {
  ...
  const unitPickerItem = page.locator('.unit-picker-item').filter({ hasText: 'Add Overlord (Characters) —' });
  await expect(unitPickerItem).toBeVisible();
  await unitPickerItem.locator('.add-unit-button').click();

  // Wait for the unit to be added
  await expect(page.locator('.unit-row')).toBeVisible();

  // Check that wargear button appears and shows correct count
  const wargearButton = page.locator('.wargear-toggle');
...
test('wargear checkboxes can be toggled and notes can be entered', async ({ page }) => {
  ...
  await expect(page.locator('.wargear-section')).toBeVisible();

  // Find the first wargear option checkbox (has choices)
  const firstCheckbox = page.locator('.wargear-option-1');
  ...
  const choiceDropdown = page.locator('.wargear-choice-1');
  ...
  const secondCheckbox = page.locator('.wargear-option-2');
  ...
  const secondChoiceDropdown = page.locator('.wargear-choice-2');
...
test('wargear selections persist after save and reload', async ({ page, request }) => {
  ...
  // Set army name
  await page.locator('.army-name-input').fill('Test Army with Wargear');
  ...
  const wargearTextBeforeSave = await page.locator('.wargear-toggle').textContent();
  ...
  await expect(page.locator('.wargear-toggle')).toContainText('1/');
  await expect(page.locator('.wargear-toggle')).toBeVisible();

  // Expand wargear section
  await page.locator('.wargear-toggle').click();

  // Verify the checkbox is still checked and choice persists
  await expect(page.locator('.wargear-option-1')).toBeChecked();
  await expect(page.locator('.wargear-choice-1')).toHaveValue('1 voidscythe');
});
