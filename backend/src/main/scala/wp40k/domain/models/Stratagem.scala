...
object StratagemId {
  ...
  given Decoder[StratagemId] = Decoder.decodeString.map(apply)
  given Schema[StratagemId] = Schema.string
}
...
object StratagemParser extends StreamingCsvParser[Stratagem] {
  ...
  protected[wp40k] def parseLine(line: String): Either[ParseError, Stratagem] = {
