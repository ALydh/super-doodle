...
object Schema {
  ...
  private val migrateChecklistNotes: ConnectionIO[Unit] = for {
    hasCol <- sql"PRAGMA table_info(armies)".query[(Int, String, String, Int, Option[String], Int)].to[List].map(_.exists(_._2 == "checklist_notes"))
