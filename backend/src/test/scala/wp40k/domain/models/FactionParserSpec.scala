...
import wp40k.errors.{InvalidId, MissingField}
...
class FactionParserSpec extends AnyFlatSpec with Matchers with EitherValues {
  ...
  "parseLine" should "parse valid faction CSV line" in {
    val line = "AoI|Imperial Agents|https://wp40k.ru/wh40k10ed/factions/imperial-agents|"
    val result = FactionParser.parseLine(line)

    result.value.id shouldBe FactionId("AoI")
    result.value.name shouldBe "Imperial Agents"
    result.value.link shouldBe "https://wp40k.ru/wh40k10ed/factions/imperial-agents"
  }

  it should "parse faction with 2-letter ID" in {
    val line = "AM|Astra Militarum|https://wp40k.ru/wh40k10ed/factions/astra-militarum|"
    val result = FactionParser.parseLine(line)
  ...
  it should "parse faction with 3-letter ID" in {
    val line = "GC|Genestealer Cults|https://wp40k.ru/wh40k10ed/factions/genestealer-cults|"
    val result = FactionParser.parseLine(line)
  ...
  it should "handle trailing pipe delimiter" in {
    val line = "NEC|Necrons|https://wp40k.ru/wh40k10ed/factions/necrons|"
    val result = FactionParser.parseLine(line)
