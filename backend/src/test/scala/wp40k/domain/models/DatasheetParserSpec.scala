...
import wp40k.errors.{InvalidFormat, MissingField}
import wp40k.domain.types.{DatasheetId, FactionId, SourceId, Role}
...
class DatasheetParserSpec extends AnyFlatSpec with Matchers with EitherValues {
  ...
  "parseLine" should "parse complete datasheet with all fields" in {
    val line = "000000884|Venerable Land Raider|AC|000000024|Land Raiders are heavily armed...|Other|<b>This model is equipped with:</b> 2 godhammer lascannons.|This model has a transport capacity of 6.|false|||1-5|While this model has 1-5 wounds remaining...|https://wp40k.ru/test|"
  ...
  it should "parse datasheet with empty legend" in {
    val line = "000001822|X-101|AdM|000000371||Characters|<b>This model is equipped with:</b> grav-gun; hydraulic claw.||false|||||https://wp40k.ru/test|"
    val result = DatasheetParser.parseLine(line)
  ...
  it should "parse virtual datasheet with empty fields" in {
    val line = "000003708|Example Wargear|SM||||||true|||||https://wp40k.ru/test|"
    val result = DatasheetParser.parseLine(line)
  ...
  it should "parse datasheet with empty loadout" in {
    val line = "000002619|Aegis Defence Line|AM|000000016|Aegis Defence Lines are barricades...|Fortifications|||false|||||https://wp40k.ru/test|"
    val result = DatasheetParser.parseLine(line)
  ...
  it should "reject invalid datasheet ID" in {
    ...
    result.left.value shouldBe a[wp40k.errors.InvalidId]
